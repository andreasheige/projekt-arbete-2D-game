...
export default function Tomato(props: GameObjectProps) {
    const name = `tomato-${props.x}-${props.y}`; // fallback name required for persisted flag
    return (
        <GameObject name={name} persisted {...props}>
            <Sprite {...spriteData.food} state="tomato" />
            <Collider isTrigger />
