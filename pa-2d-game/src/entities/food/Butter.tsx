...
export default function Butter(props: GameObjectProps) {
    const name = `butter-${props.x}-${props.y}`; // fallback name required for persisted flag
    return (
        <GameObject name={name} persisted {...props}>
            <Sprite {...spriteData.food} state="butter" />
            <Collider isTrigger />
