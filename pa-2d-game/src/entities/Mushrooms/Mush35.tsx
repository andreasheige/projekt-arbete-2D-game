...
export default function Mush35(props: GameObjectProps) {
    const name = `mush35-${props.x}-${props.y}`; // fallback name required for persisted flag
    return (
        <GameObject name={name} persisted {...props}>
            <Sprite {...spriteData.mushrooms} state="mush35" />
            <Collider isTrigger />
            <ScoreScript scoreChange={-5} />
